/*
 * FrequencyDomainConvolverO5.cpp
 *
...
FrequencyDomainConvolverO5::FrequencyDomainConvolverO5() {
...
AudioFile *FrequencyDomainConvolverO5::convolve(AudioFile *dryFile, AudioFile *irFile) {
	...
	int twoL = L + L;
	double *newDryData = zeroPad(dryData, drySize * 2, twoL);
	double *newIrData = zeroPad(irData, irSize * 2, twoL);
	delete[] dryData;
	delete[] irData;
	...
	double *resultData = new double[twoL];

	int i = 0;
	for(i = 0; i < twoL - 4; i += 6)
	{
		int iPlusOne = i+1;
		int iPlusTwo = i+2;
		int iPlusThree = i+3;
	...
	if (i == twoL - 2) {
		resultData[twoL-2] = newDryData[twoL-2] * newIrData[twoL-2] - newDryData[twoL-1] * newIrData[twoL-1];
		resultData[twoL-1] = newDryData[twoL-2] * newIrData[twoL-1] + newDryData[twoL-1] * newIrData[twoL-2];
	} else if (i == twoL - 4) {
		resultData[twoL-2] = newDryData[twoL-2] * newIrData[twoL-2] - newDryData[twoL-1] * newIrData[twoL-1];
		resultData[twoL-1] = newDryData[twoL-2] * newIrData[twoL-1] + newDryData[twoL-1] * newIrData[twoL-2];

		resultData[twoL-4] = newDryData[twoL-4] * newIrData[twoL-4] - newDryData[twoL-3] * newIrData[twoL-3];
		resultData[twoL-3] = newDryData[twoL-4] * newIrData[twoL-3] + newDryData[twoL-3] * newIrData[twoL-4];
	}

	fft(resultData - 1, L, -1);
	int expectedFinalLength = drySize + irSize - 1;
	for (int i = 0; i < twoL, i < expectedFinalLength * 2; i += 2) {
		result->addSignal(resultData[i] / (twoL));
	}
...
double *FrequencyDomainConvolverO5::vectorOfNormalToArrayOfComplex(std::vector<double> input) {
...
void FrequencyDomainConvolverO5::fft(double *data, int nn, int isign) {
...
int FrequencyDomainConvolverO5::closestPowerOfTwo(int N, int M) {
	int k = std::max(N, M);
	if (k <= 2) return 2;
	if (k <= 4) return 4;
	if (k <= 8) return 8;
...
double *FrequencyDomainConvolverO5::zeroPad(double *array, int currentSize, int wantedSize) {
